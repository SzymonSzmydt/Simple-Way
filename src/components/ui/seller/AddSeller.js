...
export function AddSeller() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (users.username.length < 3) {
      ...
      dispatch({
        type: "error",
        payload: "Nazwisko musi posiadać więcej znaków",
      });
  ...
  const infoText =
    "Utwórz podmiot, którego będziesz używać do wystawiania dokumentów sprzedażowych";
  ...
  return (
    <Window>
      <BigWindowTitleInfo infoText={infoText}>
        <SmallTitleWindow
          style={style}
          windowTitle={isValid ? "Uzupełnij pola" : error}
        >
